...
listProduct.forEach((options) => {
  options.addEventListener("click", function () {
    ...
    photoProduct.src = `/logINproject/products/images/${options.value}.jpg`;
...
listDrivers.forEach((options) => {
  options.addEventListener("click", function () {
    ...
    photoDriver.src = `/logINproject/products/images/${options.value}.jpg`;
  });
});
...
placeRiding.forEach((options) => {
  options.addEventListener("click", function () {
    ...
    placeVideo.src = `/logINproject/images/videos/${options.value}.mp4`;
    console.log(options.value);
  });
});
